...
Derived from sdks/python/apache_beam/examples/windowed_wordcount.py
Augmented with in-memory source
"""
...
if __name__ == '__main__':
    ...
    # for portable runner
    argv = sys.argv + PORTABLE_RUNNER_OPTIONS
    ...
    with beam.Pipeline(argv=pipeline_args) as p:
        ...
        lines = p | 'Create' >> beam.Create(INPUT) | beam.Map(lambda x: window.TimestampedValue(x[0], x[1]))
        ...
        # output to console (Flink task manager log)
        transformed | beam.Map(lambda x: logging.info("Got %s", x))
